...
# 메뉴 가격 최대값, 최소값
select
    max(tbl_menu.menu_price) as 'max price', min(tbl_menu.menu_price) as 'min price'
from
    tbl_menu;
...
select
    count(*), # 전체 행 개수
    count(tbl_category.ref_category_code) # null 제외
from tbl_category;
...
select
    tbl_category.category_name, count(*)
from tbl_menu
join tbl_category on tbl_menu.category_code = tbl_category.category_code
where orderable_status = 'Y'
and tbl_menu.category_code in (4, 10)
group by tbl_category.category_code;
