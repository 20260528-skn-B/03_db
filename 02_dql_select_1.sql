...
select
    ...
    menu_price + menu_price * 0.1 "부가세 포함 전체 가격"
from
    tbl_menu;
...
# 기준으로 삼은 컬럼이 여러 개 존재하면 크룹화된 정렬이 수행
# 메뉴 테이블에서
# 메뉴명, 카테고리코드, 가격을 조회
...
where
    ...
    # menu_name like '%밥'; # '밥'으로 끝나느 메뉴 조회
    # menu_name like '%마늘%'; # '마늘'이 포함된 메뉴 조회
    menu_name like '_어___' ; # 메뉴명이 5글자, 2번째 글자가 '어' 인 메뉴 조회
...
# 카테고리 테이블에서 ref_category_code가 null
select
    *
from
    tbl_category
where
    # ref_category_code 컬럼 안에 'null'이란 문자열 값이 있으면 True
...
# limit offset, n : offset 행만큼 건너뛰고 n행 만큼 조회
